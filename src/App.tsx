  ...
  return (
    <div className="h-screen flex bg-[var(--bg-base)] text-[var(--text-primary)] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="px-4 pb-3">
          <button
            ...
            className="w-full flex items-center justify-center gap-3 h-11 px-6 rounded-md border border-dashed border-[var(--border-default)] hover:border-[var(--accent)] bg-transparent hover:bg-[var(--accent-muted)] transition-all cursor-pointer group"
          >
            <svg className="w-4 h-4 text-[var(--text-tertiary)] group-hover:text-[var(--accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
            </svg>
            <span className="text-[var(--text-secondary)] text-sm font-medium group-hover:text-[var(--text-primary)]">Add Media</span>
            <span className="text-[var(--text-tertiary)] text-xs">or drag files anywhere</span>
          </button>
        </div>

        {/* Timeline */}
        <div className="h-28 border-t border-[var(--border-subtle)] bg-[var(--bg-surface)]">
          <MediaTimeline />
        </div>
      </div>

      {/* Right panel - Controls */}
      <div className="w-72 border-l border-[var(--border-subtle)] bg-[var(--bg-surface)] flex flex-col overflow-hidden">
      ...
      {ffmpegAvailable === false && (
        <div className="fixed bottom-4 left-4 right-4 p-4 bg-[var(--bg-surface)] border border-[var(--error)]/30 rounded-md">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-[var(--error)] font-medium text-sm">FFmpeg not found</h3>
              <p className="text-sm text-[var(--text-secondary)] mt-1">
                Please install FFmpeg to use this application.
              </p>
              <code className="block mt-2 px-3 py-2 bg-[var(--bg-elevated)] rounded text-xs text-[var(--text-secondary)] font-mono">
                brew install ffmpeg
              </code>
            </div>
          </div>
      ...
      {isDraggingOver && (
        <div className="fixed inset-0 z-50 bg-[var(--bg-base)]/95 backdrop-blur-sm flex items-center justify-center">
          <div className="border-2 border-dashed border-[var(--accent)] rounded-lg p-12 bg-[var(--accent-muted)]">
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <p className="text-lg font-medium text-[var(--text-primary)]">Drop to add media</p>
                <p className="text-[var(--text-secondary)] text-sm mt-1">Release to add videos and images</p>
              </div>
            </div>
          </div>
        </div>
